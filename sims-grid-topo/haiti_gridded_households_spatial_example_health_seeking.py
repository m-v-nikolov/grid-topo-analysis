...
spatial_manager = SpatialManager(
                                     ...
                                     generate_climate = True, # note that once you have generated climate for a set of nodes you can switch that to False to save time on simulation initialization; comment out climate parameters below 
                                     generate_migration = True, # note that once you have generated migration for a set of nodes you can switch that to False to save time on simulation initialization; comment out migration parameters below
                                     generate_load_balancing = True, # note that once you have generated load balancing for a set of nodes you can switch that to False to save time on simulation initialization
                                     generate_immune_overlays = False # this will become obsolete soon
                                 )


# set demographics, climate, migration parameters
spatial_manager.set_demographics_type("static") 
spatial_manager.set_res_in_arcsec("custom")
spatial_manager.set_climate_project_info("IDM-Hispaniola") # comment out if not generating climate
spatial_manager.set_climate_start_year("2014") # comment out if not generating climate
spatial_manager.set_climate_num_years("1") # comment out if not generating climate
spatial_manager.set_graph_topo_type("small-world-grid") # comment out if not generating migration



# read a map between node records and dtk node ids; dtk node ids are used to configure per node/cell campaign (e.g. health seeking)
# the map is in a form of an array; feel free to switch to dict...
with open (os.path.join("input", "grid_map.json"), "r") as gm_f:
    node_records = json.load(gm_f)
    
    node_labels_2_ids = {}
    for node_record in node_records:
         node_labels_2_ids[node_record["NodeLabel"]] = node_record["DtkNodeId"] 
    

# load node specific health seeking rates
with open("hs_rates_2_nodes.json", "r") as nhsr_f:
    hs_rates_2_nodes = json.load(nhsr_f) 
       

# configure node/cell specific health seeking rate
for hs_rate, node_labels in hs_rates_2_nodes.iteritems():
    dtk_node_ids = []
    for node_label in node_labels:
        if str(node_label) in node_labels_2_ids:
            dtk_node_ids.append(int(node_labels_2_ids[str(node_label)]))

    
    add_health_seeking(cb, 0*365,
                drug = ['Artemether','Lumefantrine'],
                targets = [ { 'trigger': 'NewClinicalCase', 'coverage': float(hs_rate), 'agemin':0, 'agemax':5, 'seek': 0.99, 'rate': 0},
                            { 'trigger': 'NewClinicalCase', 'coverage': float(hs_rate), 'agemin':5, 'agemax':100, 'seek': 0.85, 'rate': 0 },
                            { 'trigger': 'NewSevereCase', 'coverage': float(hs_rate), 'agemin':0, 'agemax':5, 'seek': 0.99, 'rate': 0},
                            { 'trigger': 'NewSevereCase', 'coverage': float(hs_rate), 'agemin':5, 'agemax':100, 'seek': 0.95, 'rate': 0} 
                          ],
                duration = 10,
                nodes = { "class": "NodeSetNodeList", "Node_List": dtk_node_ids })
